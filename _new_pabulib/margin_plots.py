...
def print_margin_plot(region, name, election,
    ...
    plt.ylim([0, get_max_cost(region, election.budget)*1.02])

    print(get_max_cost(region, election.budget))
    ...
    plt.title(f'{nice_name[rule]} ({nice_name[region]} | {NAMES[region][name]})', fontsize=18)
    name = name.replace('.pb', '')
    ...
    for region in regions:
        ...
        for i, name in enumerate(NAMES[region]):
            ...
            for rule in rules:
                ...
                print_margin_plot(region, name, election,
                                  winning_costs, winning_max_costs,
                                  losing_costs, losing_max_costs)

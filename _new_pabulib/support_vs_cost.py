...
if __name__ == "__main__":
    ...
    # plt.scatter(costs, supports, color='purple', alpha=0.1)
    # plt.show()
    # pcc = round(stats.pearsonr(costs, supports)[0], 4)
    # print(pcc)

    num_buckets = 5

    L = np.linspace(0, max(costs), num_buckets+1)
    print(L)
    ...
    for i, cost in enumerate(costs):
        j = 0
        # print(cost)
        while j != num_buckets-1 and L[j+1] < cost:
            j += 1
        buckets[j].append(supports[i])

    # averages = [0 for _ in range(num_buckets)]
    # for i, bucket in enumerate(buckets):
    #     if len(bucket) > 0:
    #         averages[i] = sum(bucket)/len(bucket)
    ...
    plt.xticks(size=27, rotation=30)
    plt.yticks(size=27)
    plt.xlabel('Normalized Cost', size=34)
    plt.ylabel('Support Distribution', size=32)

    # plt.hist(supports, bins=np.linspace(0,max(supports),51), color='seagreen')

    # plt.boxplot(buckets, labels=("0.0 - 0.04", "0.04 - 0.08", "0.08 - 0.12", '0.12 - 0.16', '0.16 - 0.2'))
    # plt.savefig('images/support_vs_cost/warsaw_boxplot', bbox_inches='tight')
    # plt.boxplot(buckets, labels=("0.0 - 0.08", "0.08 - 0.16", "0.16 - 0.24", '0.24 - 0.32', '0.32 - 0.4'))
    # plt.savefig('images/support_vs_cost/krakow_boxplot', bbox_inches='tight')
    # plt.boxplot(buckets, labels=("0.0 - 0.025", "0.025 - 0.05", "0.05 - 0.075", '0.075 - 0.1', '0.1 - 0.125'))
    # plt.savefig('images/support_vs_cost/wroclaw_boxplot', bbox_inches='tight')
    # plt.boxplot(buckets, labels=("0.0 - 0.2", "0.2 - 0.4", "0.4 - 0.6", '0.6 - 0.8', '0.8 - 1.0'))
    # plt.savefig('images/support_vs_cost/zabrze_boxplot', bbox_inches='tight')
    # plt.boxplot(buckets, labels=("0.0 - 0.12", "0.12 - 0.24", "0.24 - 0.36", '0.36 - 0.48', '0.48 - 0.6'))
    # plt.savefig('images/support_vs_cost/katowice_boxplot', bbox_inches='tight')
    # plt.boxplot(buckets, labels=("0.0 - 0.2", "0.2 - 0.4", "0.4 - 0.6", '0.6 - 0.8', '0.8 - 1.0'))
    # plt.savefig('images/support_vs_cost/gdansk_boxplot', bbox_inches='tight')
    # plt.boxplot(buckets, labels=("0.0 - 0.2", "0.2 - 0.4", "0.4 - 0.6", '0.6 - 0.8', '0.8 - 1.0'))
    # plt.savefig('images/support_vs_cost/czestochowa_boxplot', bbox_inches='tight')
